<template>
  <PageWrapper title="#1机组水侧脏污与胶球清洗">
    <a-card>
...
<script lang="ts">
  ...
  import { getCleanData } from '/@/api/sis/diagnosis';
  ...
  export default {
    ...
    setup() {
      ...
      async function getHistoryData() {
        ...
        spinning.value = true;
        const data = await getCleanData(body);
        console.log(data);
        ...
        setOptions(
          {
            ...
            series: [
              ...
              {
                name: '#1机组低背压综合热阻((m2.K)/W)',
                type: 'line',
                data: chartData['DBYZHRZ_1'].map((num) => {
        ...
        setOptions2(
          {
            ...
            series: [
              ...
              {
                name: '#1机组高背压综合热阻((m2.K)/W)',
                type: 'line',
                data: chartData['GBYZHRZ_1'].map((num) => {
                  return [num[0], parseFloat(num[1].toFixed(2))];
                }),
                lineStyle: {
                  width: 3,
